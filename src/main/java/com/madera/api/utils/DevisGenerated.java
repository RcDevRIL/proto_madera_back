...
/**
 * Classe permettant de généré un devis
 *
 * Elle est utilisée pour créer un devis de toute pièce à partir d'une
 * occurrence de projet en base de données
 *
...
public class DevisGenerated {
    ...
    private void loadFonts() {
        ...
        try {
            fontOpenSansBoldFont = BaseFont.createFont("src/main/resources/fonts/OpenSans-Bold.ttf", BaseFont.WINANSI,
                    BaseFont.EMBEDDED);
            fontOpenSansFont = BaseFont.createFont("src/main/resources/fonts/OpenSans-Regular.ttf", BaseFont.WINANSI,
                    BaseFont.EMBEDDED);
            fontOpenSansItalicFont = BaseFont.createFont("src/main/resources/fonts/OpenSans-Italic.ttf",
     ...
     * @param listComposant
     */
    public byte[] generate(Projet projet, DevisEtat devisEtat, Utilisateur utilisateur, Client client,
            Adresse adresseFacturation, List<Produit> listProduit, List<ProduitModule> listProduitModule,
            List<DevisEtat> listDevisEcheance, List<ModuleComposant> listModuleComposant,
        ...
        try {
            ...
            // FileOutputStream(String.format("devis_generated/devis_%s_%s.pdf", client.id,
            ...
            Paragraph urlSiteCCTP = new Paragraph(
                    new Chunk("Le cahier des clauses techniques particulières est disponible à l'adresse suivante : \n",
                            this.fontBody));
            ...
            document.add(new Chunk(String.format(
                    "Ce document à pour bout de faire un récapitulatif des différents produits, des composants de chaque module du projet %s, ainsi que de leurs modules.\n\n",
    ...
    private PdfPTable buildTableCoordinates(Date date, String refProjet, String refClient) {
        ...
        tableCoordinates.addCell(buildCell(String.format("Date : %s", Date.valueOf(LocalDate.now())),
                Element.ALIGN_RIGHT, false, false, false, false));
        tableCoordinates.addCell(buildCell("85000, Rue Enzo Ferrari, La Roche sur sur Yon", Element.ALIGN_LEFT, false,
                false, false, false));
        tableCoordinates.addCell(buildCell(String.format("Ref. projet : %s", refProjet), Element.ALIGN_RIGHT, false,
                false, false, false));
        tableCoordinates.addCell(buildCell("02 51 98 18 32", Element.ALIGN_LEFT, false, false, false, false));
        tableCoordinates.addCell(buildCell(String.format("Ref. client : %s", refClient), Element.ALIGN_RIGHT, false,
                false, false, false));
    ...
    private PdfPTable buildTableDestAndSend(Utilisateur utilisateur, Client client, Adresse adresse) {
        ...
        tableDestAndSend.addCell(buildCell(utilisateur.getNom() + " " + utilisateur.getPrenom(), Element.ALIGN_LEFT,
        ...
        tableDestAndSend.addCell(buildCell("85000, Rue Enzo Ferrari, La Roche sur sur Yon", Element.ALIGN_LEFT, false,
                false, false, false));
        tableDestAndSend.addCell(buildCell(
                adresse.getVille() + " " + adresse.getCodePostale() + ", " + adresse.getNumero() + " "
                        + adresse.getRue() + " " + adresse.getComplement(),
                Element.ALIGN_RIGHT, false, false, false, false));
     ...
     * @return
     */
    private PdfPCell buildCell(String content, int alignement, boolean isSubTitle, boolean hasBorder, boolean isItalic,
            boolean isBold) {
    ...
    private PdfPTable buildTableOfProducts(Produit produit, List<ProduitModule> listProduitModule) {
        ...
        for (ProduitModule produitModule : listProduitModule) {
            if (produit.getProduitId() == produitModule.getProduitId()) {
                tableOfProducts.addCell(buildCell(produitModule.getProduitModuleNom(), Element.ALIGN_BASELINE, false,
                        true, false, false));
                tableOfProducts.addCell(buildCell(buildDimensionModule(produitModule.getProduitModuleSectionLongueur()),
                        Element.ALIGN_BASELINE, false, true, false, false));
                tableOfProducts.addCell(buildCell(String.valueOf(Math.round(produitModule.getPrixModule())),
                        Element.ALIGN_BASELINE, false, true, false, false));
                tableOfProducts.addCell(buildCell(String.valueOf(Math.round(produitModule.getPrixModule())),
                        Element.ALIGN_BASELINE, false, true, false, false));
            }
    ...
    private PdfPTable buildTableEcheance(double prixProjet, List<DevisEtat> listEcheance) {
        ...
        for (DevisEtat devisEtat : listEcheance) {
            ...
            tableEcheance.addCell(
                    buildCell(String.format("%s (%s )", devisEtat.devisEtatLibelle, devisEtat.pourcentageSomme),
                            Element.ALIGN_LEFT, false, false, true, false));
            tableEcheance
                    .addCell(buildCell(Math.round(calculSomme) + " €", Element.ALIGN_RIGHT, false, false, false, true));
        }
        ...
        for (Produit produit : listProduit) {
            ...
            for (ProduitModule produitModule : listModule) {
                if (produitModule.getProduitId() == produit.getProduitId()) {
                    listPucesModules.add(new ListItem(
                            new Chunk(String.format("  %s", produitModule.getProduitModuleNom()), this.fontItalic)));
     ...
     * @return
     */
    private Paragraph detailProduitAndModule(List<Produit> listProduit, List<ProduitModule> listModule,
            List<Composant> listComposants, List<ModuleComposant> listModuleComposant) {
        Paragraph paragraph = new Paragraph();
        for (Produit produit : listProduit) {
            ...
            for (ProduitModule produitModule : listModule) {
                if (produit.getProduitId() == produitModule.getProduitId()) {
                    paragraph.add(new Chunk(
                            ...
                            this.fontItalic));
                    paragraph.add(
                            buildTableOfComposants(produitModule.getModuleId(), listComposants, listModuleComposant));
                }
            }
        }
        return paragraph;
     ...
     * @return
     */
    private PdfPTable buildTableOfComposants(Integer moduleId, List<Composant> listComposants,
        ...
        for (Composant composant : listComposants) {
            for (ModuleComposant moduleComposant : listModuleComposant) {
                if (moduleComposant.getComposantId() == composant.getComposantId()
                        && moduleComposant.getModuleId() == moduleId) {
                    tableOfComposants.addCell(
                            buildCell(composant.getLibelle(), Element.ALIGN_BASELINE, false, true, false, false));
                    tableOfComposants.addCell(
                            buildCell(composant.getLibelleGroupe(), Element.ALIGN_BASELINE, false, true, false, false));
                    tableOfComposants.addCell(
                            buildCell(composant.getUniteUsage(), Element.ALIGN_BASELINE, false, true, false, false));
                    tableOfComposants.addCell(buildCell(String.valueOf(composant.getSection().orElse(null)),
                            Element.ALIGN_BASELINE, false, true, false, false));
                    tableOfComposants.addCell(buildCell(String.valueOf(Math.round(composant.getPrixComposant())),
                            Element.ALIGN_BASELINE, false, true, false, false));
                }
            }
        }
