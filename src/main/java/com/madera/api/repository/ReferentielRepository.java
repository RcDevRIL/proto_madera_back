...
public class ReferentielRepository {
    ...
    // TODO Faire une seul méthode pour renvoyer les données du référentiel ? Je
    // pense fortement
    // TODO Ca évitera de surcharger le controller avec des imports dans tous les
    // sens
    ...
    public List<Projet> getAllProjects(Integer userId) {
        return context.select(PROJET.fields()).select(CLIENT.fields()).select(DEVIS_ETAT.fields()).from(PROJET)
                .join(CLIENT).using(CLIENT.I_CLIENT_ID).join(DEVIS_ETAT).using(DEVIS_ETAT.I_DEVIS_ETAT_ID)
                .join(PROJET_UTILISATEURS).on(PROJET_UTILISATEURS.I_PROJET_ID.eq(userId))
